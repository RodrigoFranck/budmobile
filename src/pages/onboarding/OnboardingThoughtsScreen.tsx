...
export default function OnboardingThoughtsScreen() {
  const insets = useSafeAreaInsets();
  const navigation = useNavigation<OnboardingNavigationProp>();
  const { initialThoughts, setInitialThoughts } = useOnboardingFlow();
  const [text, setText] = useState(initialThoughts);
  const onboardingColors = useOnboardingColors();

  const onContinue = () => {
    setInitialThoughts(text.trim());
    navigation.navigate('OnboardingExpectations');
  };
    ...
    <TouchableWithoutFeedback onPress={Keyboard.dismiss} accessible={false}>
      ...
      >
        ...
        <View className="flex-1 px-6" style={{ paddingHorizontal: H_PAD }}>
          ...
          >
            Apenas escreva algo sobre o que você tem pensado, exatamente como vier à mente.
          </Text>
          <TextInput
            value={text}
            onChangeText={setText}
            placeholder="Ultimamente eu tenho pensado em..."
            ...
            accessibilityLabel="Seus pensamentos recentes"
          />
        </View>
        <View
          style={{
